...
"""Regenerate the two data-derived reference files from the live wire (GET only):
  ...
  reference/topic-course-map.json               - topic id -> majority Timeback COURSE ID from 14 days of lesson/multistep events
...
# ---- topic -> course id map (lesson + multistep only; quiz URLs carry an instance id, reviews revisit earlier courses) ----
...
for r in res:
    ...
    m = re.search(r"/topics/(\d+)/(lesson|multistep)$", (r.get("metadata") or {}).get("originalObjectId") or "")
...
(ROOT / "reference" / "topic-course-map.json").write_text(json.dumps({
    "_about": "Math Academy topic id -> the Timeback COURSE ID (sourcedId) under which lesson and multistep events for that topic were filed by >= 5 distinct non-test students with >= 80% agreement, over the 14 days before `readAt`. Filed course comes from each event's LINE ITEM (the truthful path, DICTIONARY trap 21). Reviews are excluded because they revisit earlier courses; quizzes are excluded because their URL carries a quiz-instance id in the topic slot. HEURISTIC: adjacent grade courses share topics, so this is the course most students who met the topic were filed under, not Math Academy's catalogue. Coverage is dense for grade-level courses and thin for high-school courses. Regenerate with scripts/regen_references.py (ENABLEMENT example 7, step A).",
