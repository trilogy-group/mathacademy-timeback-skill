...
async function storeMeta() {
  ...
  return { snapshotAt, rosterReadAt: r.Item.rosterReadAt?.S || snapshotAt, snapshotAgeHours: ageHours, staleAfterDays, stale: ageHours !== null && ageHours > staleAfterDays * 24,
    schedule: { snapshot: "03:00 America/Chicago daily", activity: "03:45 America/Chicago daily, for the previous America/Chicago day", nextSnapshotDueUtc: next.toISOString(), scheduledRunsSoFar: scheduled, manualRunsSoFar: manualRuns, schedulerTestRuns: history.length - scheduled - manualRuns, firstScheduledRunHasHappened: scheduled > 0 },
    ...
    snapshotCalls: JSON.parse(r.Item.calls?.S || "{}"), readCalls: { maLookupOnMiss: num(r.Item.readMaLookup) || 0, maKnowledge: num(r.Item.readMaKnowledge) || 0, maActivityBackfill: num(r.Item.readMaActivityBackfill) || 0, maActivityBackfillStudents: num(r.Item.readMaActivityBackfillStudents) || 0, counterNote: "maActivityBackfill counts Math Academy CALLS (about five per student) since 2026-09-18 00:00Z; before that it counted students, so early figures are low; maActivityBackfillStudents counts students backfilled on demand" }, backfill: r.Item.backfill?.S ? JSON.parse(r.Item.backfill.S) : null,
...
const numish = v => (v === null || v === undefined || v === "") ? null : (/^\d+$/.test(String(v)) ? Number(v) : v);   // Math Academy ids are numbers; serve them as numbers everywhere
...
async function storeStudent(event, q, sidFromPath) {
  ...
  if (!row.Item) {
    ...
    if (email && Date.now() - lastTried > 864e5 && (await maKey())) {
      ...
      return resp(200, { ...base, inSnapshot: false, matchedBy: null, isTestUser: !!gate.body?.user?.metadata?.isTestUser, seatsAtSnapshot: seats, unmatchedReason: reason, lastTriedAt: now, mathAcademy: null, mathAcademyState: "no math academy record", courseAgreement: "no math academy record",
...
async function storeSub(event, q, sid, kind) {
  ...
  if (kind === "activity") {
    ...
    return resp(200, { sourcedId: sid, ...fresh, backfilledNow: backfill, coverage, reasonIfEmpty, daysRequested, daysRequestedNote: daysRequested ? null : "daysRequested[] is served only when the range is 90 days or fewer (or when from/to are given); ask in slices for a per-day status", from, to, dayBoundary: "America/Chicago; each day is the UTC window in days[].windowUtc, tasks kept by their Math Academy completion time", units: "every clock and epoch here is MILLISECONDS (task and day alike); divide by 60000 for minutes", activityLastDate: meta.activityLastDate || null, days, tasks, note: "Math Academy's own per-task analysis by America/Chicago day; for a backfilled student (coverage.backfilledAt set) a day absent between backfilledFrom and activityLastDate is a day with no Math Academy task; for a student not yet backfilled an absent day was simply not pulled (nightly pulls cover only students with a Timeback result that day); a day after activityLastDate is not pulled yet; the current day is partial; daysRequested[] gives the status per day; ENABLEMENT example 11" });
  }
  if (kind === "knowledge") {
    const row = await ddb.send(new GetItemCommand({ TableName: ST, Key: { pk: S("student"), sk: S(sid) } }));
...
export const handler = async (event) => {
  ...
  if (crsAct && method === "GET") {
    ...
    if (day > todayChiC) return resp(200, { courseSourcedId: cid, date: day, activityLastDate: meta.activityLastDate || null, snapshotAt: meta.snapshotAt, stale: meta.stale, rows: [], note: "that day has not happened yet in America/Chicago" });
    ...
    return resp(200, { courseSourcedId: cid, date: day, dayStatus: partialDay ? "partial: after activityLastDate (the current or an unpulled day); rows here come from on-demand reads and will grow" : "pulled", dayBoundary: "America/Chicago", windowUtc: chicagoWindowUtc(day), units: "milliseconds", activityLastDate: meta.activityLastDate || null, snapshotAt: meta.snapshotAt, stale: meta.stale, coverage: { studentsBackfilled: backfilledN, studentsNotBackfilled: members.length - backfilledN, note: day < "2026-09-15" ? "this day is before the nightly activity store began (2026-09-15): rows exist ONLY for backfilled students, so a class total here covers studentsBackfilled of studentsWithStoreRow; if studentsNotBackfilled is not 0, the class figure is partial and you should say so (the per-student /activity route backfills a student on first ask)" : "nightly-pulled day: every roster student with a Timeback result that day was pulled, backfilled or not" }, rosterStudents: rosterCount, rosterNonTest: meta.byCourse[cid]?.nonTest?.students ?? null, studentsWithStoreRow: members.length, rosterWithoutMathAcademyRecord: rosterCount === null ? null : rosterCount - members.length, studentsWithActivity: good.length, studentsWithError: out.length - good.length, totals: { numTasks: sum("numTasks"), timeElapsedMs: sum("timeElapsedMs"), timeEngagedMs: sum("timeEngagedMs"), timeProductiveMs: sum("timeProductiveMs"), xpAwarded: sum("xpAwarded") }, rows: out, note: "one row per roster student of this Timeback course who had a pulled activity day on `date` (Math Academy's own per-task analysis summed by the store); rosterStudents is the course roster at rosterReadAt (test users included; rosterNonTest excludes them), studentsWithStoreRow those with a Math Academy record, and the denominator for 'share of my class active' is rosterNonTest; rows carry isLikelyTest so you can drop synthetic accounts; students absent from rows had no Math Academy result in Timeback that day, or (before 2026-09-15) were not yet backfilled: read coverage before quoting a class total; totals sum the rows without error; opaque ids only; ENABLEMENT example 11" });
  }
