...
const STATUS_NOTES = {
  ...
  staleness: "stale is true once snapshotAgeHours exceeds staleAfterDays*24 (2 days once nightly runs; a missed night shows here first); past that, quote mathAcademy figures only with their date and prefer Timeback's live containers (ENABLEMENT A3) for progress.",
...
export const handler = async (event) => {
  ...
  if (crsAct && method === "GET") {
    ...
    return resp(200, { courseSourcedId: cid, date: day, dayStatus: partialDay ? "partial: after activityLastDate (the current day, or a day the nightly has not pulled); rows here come from the backfill batch or from on-demand reads and will grow until the nightly pull completes the day; never publish this day as complete" : "pulled", dayBoundary: "America/Chicago", windowUtc: chicagoWindowUtc(day), units: "milliseconds", activityLastDate: meta.activityLastDate || null, snapshotAt: meta.snapshotAt, stale: meta.stale, coverage: { studentsBackfilled: backfilledN, studentsNotBackfilled: members.length - backfilledN, note: day < "2026-09-15" ? "this day is before the nightly activity store began (2026-09-15): rows exist ONLY for backfilled students, so a class total here covers studentsBackfilled of studentsWithStoreRow; if studentsNotBackfilled is not 0, the class figure is partial and you should say so (the per-student /activity route backfills a student on first ask)" : "nightly-pulled day: every roster student with a Timeback result that day AS OF THE PULL TIME (see pulledAt) was pulled, backfilled or not; results Timeback ingests after the pull (late Caliper rows, a few per estate-day) land on the next night's re-pull of this day, so a day can grow by a student or two until the night after next" }, pulledAt: partialDay ? null : (meta.lastActivityRun?.finishedAt || null), rosterStudents: rosterCount, rosterNonTest: meta.byCourse[cid]?.nonTest?.students ?? null, studentsWithStoreRow: members.length, rosterWithoutMathAcademyRecord: rosterCount === null ? null : rosterCount - members.length, studentsWithActivity: good.length, studentsWithError: out.length - good.length, totals: { numTasks: sum("numTasks"), timeElapsedMs: sum("timeElapsedMs"), timeEngagedMs: sum("timeEngagedMs"), timeProductiveMs: sum("timeProductiveMs"), xpAwarded: sum("xpAwarded"), questions: sum("questions"), questionsCorrect: sum("questionsCorrect") }, rows: out, note: "one row per roster student of this Timeback course who had a pulled activity day on `date` (Math Academy's own per-task analysis summed by the store); rosterStudents is the course roster at rosterReadAt (test users included; rosterNonTest excludes them), studentsWithStoreRow those with a Math Academy record, and the denominator for 'share of my class active' is rosterNonTest; rows carry isLikelyTest so you can drop synthetic accounts; students absent from rows had no Math Academy result in Timeback that day, or (before 2026-09-15) were not yet backfilled: read coverage before quoting a class total; totals sum the rows without error; opaque ids only; ENABLEMENT A2" });
  }
  ...
  if (p === "/store/finishers" && method === "GET") {
    ...
    const meta = await storeMeta(); if (!meta) return resp(503, { error: "no snapshot loaded" });
    const since = q.since || "2000-01-01";
