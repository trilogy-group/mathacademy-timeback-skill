...
const STATUS_NOTES = {
  ...
  readCalls: "estate-wide (all readers) count of Math Academy calls this front has made on readers' behalf since the store began: maLookupOnMiss (a per-student read of a student missing from the store, once per 24 h), maKnowledge (one per student per courseId per 7 days), maActivityBackfill (the FIRST /activity read of a student never backfilled: about five calls once, then never). After that first read, /activity, /history and the course routes never reach Math Academy and may be looped.",
...
export const handler = async (event) => {
  ...
  if (crsAct && method === "GET") {
    ...
    if (!meta.byCourse[cid]) return resp(404, { error: "no roster student sat in that Timeback course at snapshot time (or unknown course id)", snapshotAt: meta.snapshotAt });
    if (day > (meta.activityLastDate || "")) return resp(200, { courseSourcedId: cid, date: day, activityLastDate: meta.activityLastDate || null, snapshotAt: meta.snapshotAt, stale: meta.stale, rows: [], note: "that day has not been pulled yet (after activityLastDate); rows will exist after the next nightly run" });
    const members = [];
    ...
    return resp(200, { courseSourcedId: cid, date: day, dayBoundary: "America/Chicago", units: "milliseconds", activityLastDate: meta.activityLastDate || null, snapshotAt: meta.snapshotAt, stale: meta.stale, rosterStudents: rosterCount, rosterNonTest: meta.byCourse[cid]?.nonTest?.students ?? null, studentsWithStoreRow: members.length, rosterWithoutMathAcademyRecord: rosterCount === null ? null : rosterCount - members.length, studentsWithActivity: good.length, studentsWithError: out.length - good.length, totals: { numTasks: sum("numTasks"), timeElapsedMs: sum("timeElapsedMs"), timeEngagedMs: sum("timeEngagedMs"), timeProductiveMs: sum("timeProductiveMs"), xpAwarded: sum("xpAwarded") }, rows: out, note: "one row per roster student of this Timeback course who had a pulled activity day on `date` (Math Academy's own per-task analysis summed by the store); rosterStudents is the course roster at rosterReadAt (test users included; rosterNonTest excludes them), studentsWithStoreRow those with a Math Academy record, and the denominator for 'share of my class active' is rosterNonTest minus test rows; students absent from rows had no Math Academy result in Timeback that day or the day was not pulled; opaque ids only; ENABLEMENT example 11" });
  }
