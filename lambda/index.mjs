...
const finished = cc => !!cc?.completed && ((cc.progress === null || cc.progress === undefined) || Number(cc.xpRemaining ?? 0) < 100);   // owner ruling 2026-09-18: a completed stamp with 100+ XP left is not a finish
const maState = cc => !cc ? null : finished(cc) ? "completed" : cc.completed ? "completed flag set, not finished" : (cc.progress === null || cc.progress === undefined) ? "in progress" : cc.progress >= 0.995 ? "at 100, not marked complete" : cc.progress === 0 ? "not started" : "in progress";
...
const agreementOf = (ma, seats) => { const name = ma?.currentCourse?.name; if (!name) return "math academy has no current course"; if (!seats.length) return "no current timeback seat"; const done = finished(ma.currentCourse); return seats.some(s => normCourse(s.courseName) === normCourse(name)) ? (done ? "agree, math academy course completed" : "agree") : (done ? "disagree, math academy course completed" : "disagree"); };
...
export const handler = async (event) => {
  ...
  if (crs && method === "GET") {
    const auth = await tokenOk(event);
    if (!auth.ok) return resp(auth.status === 401 ? 401 : (auth.status >= 400 && auth.status < 500 ? auth.status : 502), { error: auth.status === 401 && !(event.headers?.authorization || event.headers?.Authorization) ? "per-course counts are served only with the reader's Timeback token (Authorization: Bearer <token>)" : "Timeback did not accept the token", timebackStatus: auth.status });
    const meta = await storeMeta(); if (!meta) return resp(404, { error: "no snapshot loaded" });
  ...
  if (p === "/store/finishers" && method === "GET") {
    ...
    for (const it of await queryAll("student")) {
      ...
      const completedDay = finished(cc) ? String(cc.completed).slice(0, 10) : null; const flagOnly = !!cc.completed && !finished(cc); const at100 = !cc.completed && Number(cc.progress) >= 0.995;
      ...
      const flagDay = flagOnly ? String(cc.completed).slice(0, 10) : null;
      if (!(completedDay && completedDay >= since) && !(flagDay && flagDay >= since) && !at100 && !nearOff) continue;
      const bucket = completedDay ? "completed" : flagOnly ? "completed flag set, not finished" : at100 ? "at 100, not marked complete" : "left the roster near the end, not finished";
    ...
    const finishedRows = rows.filter(r => r.bucket === "completed" || r.bucket === "at 100, not marked complete");
    return resp(200, { since, mathAcademyCourseId: wantCourse ? Number(wantCourse) : null, snapshotAt: meta.snapshotAt, stale: meta.stale, count: rows.length, nonTestCount: rows.filter(r => !r.isLikelyTest).length, finishedCount: finishedRows.length, finishedNonTestCount: finishedRows.filter(r => !r.isLikelyTest).length, workedThroughNonTestCount: finishedRows.filter(r => !r.isLikelyTest && !r.passThrough).length, countNote: "count and nonTestCount are ALL rows including the near-finisher and flag-only buckets; finishedCount and finishedNonTestCount are the completed and at-100 buckets; workedThroughNonTestCount also drops the likely placement pass-throughs (under 20 lessons in the course)", byBucket: rows.reduce((a, r) => (a[r.bucket] = (a[r.bucket] || 0) + 1, a), {}), rows,
      bucketNote: "completed = Math Academy set the flag on or after since AND fewer than 100 XP remain (owner rule 2026-09-18; on SAT Math Prep the flag alone counts because its XP figure is unreliable); completed flag set, not finished = the flag is set but 100 or more XP remain (Math Academy has stamped it at 69 percent and at 81 percent on students who kept working for months; meaning unknown, never a finish); (read progress and daysInCourse beside a completed row: a completed after a few days and a few tasks is a placement pass-through, not a course worked through; a completed under 0.9 is either a course switch or an early close by an administrator (two students have sat 15 months at 69 percent and at 81 percent with the flag): not decidable here); at 100, not marked complete = progress >= 0.995 without the flag, served whatever the date; left the roster near the end, not finished = no current Timeback seat, progress 0.95 to 0.994, no flag: Timeback's rounded 100 progressed them out before Math Academy closed the course, so they appear in no course list and no day total",
  ...
  return resp(404, { error: "not found", routes: ["GET /skill", "GET /about", "GET /DICTIONARY.md", "GET /ENABLEMENT.md", "GET /reference/<file>.json", "GET /feedback[?state=]", "GET /feedback/{n}", "POST /feedback", "GET /store", "GET /store/courses", "GET /store/finishers?since=", "GET /store/course/{courseSourcedId}", "GET /store/course/{courseSourcedId}/students[?agreement=]", "GET /store/course/{courseSourcedId}/activity?date=", "GET /store/student/{sourcedId}[?minimal=1]", "GET /store/student?email=", "GET /store/student/{sourcedId}/activity", "GET /store/student/{sourcedId}/courses", "GET /store/student/{sourcedId}/history", "GET /store/student/{sourcedId}/knowledge"] });
};
