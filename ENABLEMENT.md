...
A. GET <front base>/store/finishers?since=<YYYY-MM-DD>[&mathAcademyCourseId=<id>]      # every store row (roster or not) whose CURRENT Math Academy course carries completed >= since, sits at >= 0.995, or (no seat) sits at 0.95 to 0.994; a few seconds
      #   counts: count / nonTestCount = ALL rows incl. the near-finisher and flag-only buckets; finishedCount / finishedNonTestCount = completed + at-100 buckets; workedThroughNonTestCount also drops likely placement pass-throughs; byBucket
...
- **Finished = Math Academy's stamp with fewer than 100 XP remaining** (owner rule 2026-09-18), or `at 100, not marked complete`. The stamp alone is not a finish: Math Academy has set it at 69, 81 and even 0 percent on students who then worked the course for months, so those rows are served as `completed flag set, not finished` and are never counted. On SAT Math Prep the stamp alone counts (its XP figure is unreliable). Report `progress` and `xpRemaining` beside every completion date. `completed` is usually stamped at the student's last lesson, seconds after it.
