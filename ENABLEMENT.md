...
C. GET /ims/oneroster/gradebook/v1p2/assessmentLineItems/<id> for each distinct line item in B   # filed course per event: ONE GET PER EVENT. A student with a year of history has a thousand results, and this step is a thousand calls.
      # Affordable form: attribute by line item only inside the window you are asked about (60 to 90 days); for the earlier history, group results by scoreDate against the seats' begin/end dates and label those rows "attributed by seat dates, not by line item" (trap 21 says this can be wrong for students moved between courses). The store's history (D) is the forward-looking substitute from 2026-09-17 on. A results filter on the line item's course is rejected (trap 19), so there is no cheaper exact path.
