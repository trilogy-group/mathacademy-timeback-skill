...
- Estimated XP remaining: `course.metadata.metrics.totalXp × (1 − pct/100)`, basis "timeback course XP", per the rules in `reference/course-xp-size.json` (no estimate when the percent belongs to another course, when the course carries no `totalXp`, when no percent exists yet, or on SAT Math Prep). **The estimate runs low, sometimes by more than half**: on the students checked it was a quarter to two thirds below Math Academy's exact figure. Where the store holds the student, its `xpRemaining` is the exact figure as of `figuresAsOf`; prefer it, and show the estimate only as the Timeback-only fallback with its bias named. **State the bias**: the estimate runs low for most students, typically by a fifth to nearly half, because Math Academy awards extra XP on failed reviews; present it as a band, never as the figure. Never present it as Math Academy's number.
...
- **Freshness rule.** Read `snapshotAt`, `snapshotAgeHours`, `stale` and `schedule` first. The store refreshes nightly (03:00 America/Chicago); `history[]` on `/store` lists every snapshot ever loaded with its `source` (`schedule` or `manual`), `schedule.firstScheduledRunHasHappened` says whether the cron has ever fired, `schedule.nextSnapshotDueUtc` when the next is due, and `lastActivityRun` what the last activity pull did (day, students, errors, calls, the UTC window). While `stale` is false (within `staleAfterDays`, two days as served), quote `xpRemaining`, `progress`, `grade` and `currentCourse` as "as of <snapshotAt>". Once `stale` is true (a night was missed), still quote them only with their date, take progress from the newest result instead (ex. 3), and treat `courseAgreement` as indicative: the Timeback half is live, the Math Academy half is old. `figuresAsOf` on a row is `snapshotAt` for rows Math Academy answered directly (`lookup`, `live-lookup`) and "up to one day before" for rows taken from Math Academy's bulk list. A stuck refresh is a ticket on the feedback wire.
...
- **Cohort questions** go to `/store/course/{courseSourcedId}` (counts by `courseAgreement`, `testUsers`, `maNotStarted`, one call, no token) and, for the rows behind a count, `/store/course/{courseSourcedId}/students?agreement=<value>` (your token; opaque `user.sourcedId`s with their store labels, no names, no figures). Per-student rows cost about one second each (two Timeback calls replayed) **and a miss costs one Math Academy call**, so never loop the per-student route over a roster to find the unmatched: the list route answers that in one call.
- **What reaches Math Academy at read time:** nothing, except one live lookup when a student is missing from the store (once per 24 h per student) and one knowledge-map fetch per student per course per 7 days. `/store` counts both under `readCalls`.
...
- `inSnapshot: false` is not "no Math Academy account": read `unmatchedReason` (HTTP 404 = no account under our key; HTTP 401 = under another organisation's key; `no email`) and `lastTriedAt`. On a miss the route makes **one live Math Academy lookup** (then not again for 24 h): a found student comes back as `matchedBy: "live-lookup"` and is carried by every later night; a not-found one keeps the HTTP reason. Unmatched rows still carry `isTestUser` and `seatsAtSnapshot`. A record fresher than last night needs the one-off call in § One-off exact figures with a Math Academy key.
- `figuresAsOf` is the timestamp of the read that produced the Math Academy figures; `figuresBasis` says whether Math Academy answered a direct per-student call (exact at that moment) or the figures come from its bulk list (which can lag the live record by up to a day).
- Math Academy's own record is copied as served, artefacts included: `schedule.startDate` has been seen as the literal string `Invalid date` on a just-created account. Treat any non-date string in a date field as null.
...
- **The three clocks.** `timeElapsed` is wall time on the task (the only clock Timeback receives, as `activeSeconds`); `timeEngaged` is the part Math Academy judged attentive; `timeProductive` is the part that advanced the task. Report the ratios per day, not per task: a single review is seconds long. Aggregate labels: numerator and denominator in minutes, basis = tasks Math Academy recorded on the pulled days.
...
- Failure mode: treating a day with no `days[]` row as a day of zero work; reading any clock here as seconds; reconciling with Timeback by UTC date instead of by task id; reading `history` as Math Academy's full past when it starts at the store's first night.
...
- `knowledge.courses[]` is the course asked for plus up to two prerequisite courses, **already included**: asking for a prerequisite by `courseId` spends another live call for the same data. Walk `units[] → modules[] → topics[]`; `stability` (0–1) is Math Academy's estimate of long-term retention, averaged upward to module and unit; `completion` (0–1) per course. Read low but non-zero stability on a studied topic as "fading"; **a stability of exactly 0 is Math Academy's value where it has no retention evidence yet**, so ties at 0 mean "not yet shown", not "forgotten", and the "weakest topics" list should exclude exact zeros or label them (open question #12). A topic absent from every course was never taught here.
...
Output shape: per course — `courseId`, `name`, `completion`; per unit — `name`, `stability`, `weakestTopics[] {topicId, name, stability}` (lowest non-zero first, ties broken by curriculum order, exact zeros listed separately as `notYetRated`); a headline `topicsBelow(0.5)` count with its denominator and the zero count beside it.
