...
| Store | `GET …/store` (freshness, schedule, health open; counts and byCourse with a token); `GET …/store/course/{id}` (token; + `/students[?agreement=]`, `/activity?date=`), `GET …/store/courses` (open); `GET …/store/student/{sourcedId}[?minimal=1]` (or `?email=`) + `/history`, `/activity?from&to`, `/knowledge` with `Authorization: Bearer <reader's Timeback token>`; the Lambda replays the token against Timeback and serves only students it answers 200 for; Math Academy is called only for the knowledge map (cached 7 d) and a live lookup on a store miss |
...
| Alerts | SNS topic `mathacademy-timeback-alerts` (email to the owner) fed by CloudWatch alarms `mathacademy-timeback-nightly-errors`, `mathacademy-timeback-nightly-missed` (no invocation in a UTC day), `mathacademy-timeback-skill-errors`; `py -3 lambda/nightly/alarms.py --check`. GitHub channel: `.github/workflows/health-check.yml` reads `GET /store` `health` daily at 10:40 UTC and opens/updates a `store-health` issue when `ok` is false (GitHub emails the repo's watchers) |
