...
- `metadata.metrics.totalXp`, `totalLessons`, `totalGrades` — Timeback's running totals for this enrollment. **computed** by Timeback from the events it received. `totalXp` here is not Math Academy's XP-to-complete for the course (trap 11).
...
- **Estimated XP remaining in the current course.** Math Academy does not send its XP-remaining figure to Timeback. The rule set lives in `reference/course-xp-size.json`, keyed by Timeback `course.sourcedId` (never title), one row per course in the domain above. In order:
  ...
  3. If the row has no `sizeXp`: `remaining_lower ≈ course.metadata.metrics.totalXp × (1 − pctCompleteApp / 100)`, basis "timeback lower bound". Every input is on the wire, but Timeback's course `totalXp` is a different, smaller quantity than Math Academy's course size (Timeback's figure is the XP credited toward the course; Math Academy also awards XP for reviews, retries and quizzes), so this is a floor and must be labelled one.
  4. If the row's `use` is `never` (SAT Math Prep only): no estimate. Math Academy reports an estimated SAT score on that course instead of progress, a figure Timeback never receives, and `pctCompleteApp` there is stale or absent. SAT Math Fundamentals still reports progress and is sized like any other course.
...
11. **Timeback's course `totalXp` is not Math Academy's course size.** The two are different quantities set by different systems; using Timeback's figure as the denominator for progress or for XP remaining under-counts. Use `pctCompleteApp` for progress and the calibrated sizes (or the labelled lower bound) for XP remaining. Measure: for one course, compare `course.metadata.metrics.totalXp` with the median over its students of `enrollment.metadata.metrics.totalXp ÷ (pctCompleteApp/100)`.
