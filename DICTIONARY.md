...
- **The Timeback course an event is filed under is the course of the Timeback enrollment the platform matched at ingestion, never the course Math Academy says the student is in, and not necessarily the student's CURRENT enrollment either.** `[platform-note]`, verified by probe. The filed course is what the event's line item carries (`assessmentLineItem.course.sourcedId`). On a sample day it disagreed with the student's current active enrollment for a few percent of rows: those rows were filed under an older enrollment since marked `tobedeleted`, and in one case under a course the student was never enrolled in. Two consequences, both load-bearing: (a) if the enrollment is wrong, every event is filed under the wrong course and no field on the event says so (trap 10, ENABLEMENT example 7); (b) to know where an event was filed, read its line item, never the student's current enrollment (trap 21, ENABLEMENT example 8).
...
- **native:** list `GET /ims/oneroster/gradebook/v1p2/assessmentResults` with `filter`; instance `GET .../assessmentResults/{sourcedId}`. Filters honoured (probed 2026-09-16/17): `student.sourcedId='…'`, `scoreDate>='<ISO>'`, `scoreDate<='<ISO>'`, `metadata.appName='Math Academy'`, `sourcedId~'caliper_'`, combined with ` AND `; `sort=scoreDate&orderBy=desc|asc`. Filter on `assessmentLineItem.course.sourcedId` is **rejected** (400, "Error in filter query"); to scope by course, list line items by course or by `dateLastModified>=` and join (see relationships, trap 19).
...
- **native:** instance `GET /ims/oneroster/gradebook/v1p2/assessmentLineItems/{sourcedId}`; list `GET .../assessmentLineItems?filter=course.sourcedId='…'` (honoured; large: order 10,000 rows per popular course, paginate at 3000) or `?filter=dateLastModified>='<day>'` (honoured, observed 2026-09-17; returns every app's line items for the day, order 100,000s of rows, so filter to the result ids you hold).
...
- **one instance:** a class is one section of a course; a course is the catalogue object. Math Academy classes are titled `Math Academy - <course> Class`, are `metadata.is_automanaged: true` (created by the platform, not by staff), and there are many sections per course (trap 1). **captured.**
- **id / identity:** `sourcedId` on both; the class references `course.sourcedId` and `school.sourcedId` (the org that owns the section; the class's own `org` field is null on every row observed). The course carries `org.sourcedId`. Course ids are the stable key. Titles are not (trap 1).
...
- `metadata.totalQuestions`, `metadata.correctQuestions` — question counts on the task. **copied.** On results, `correctQuestions` was present on every row observed on the sample day, 0 on the null-score rows (trap 9); it is the EduBridge weekly fact rows that carry `correctQuestions: null`.
...
1. **Class and course titles are not canonical, and one title names many classes.** One course title, `Math Academy - Beyond AI`, names two course ids; one course is titled just `Math Academy`; the `8th grade` course lives in a different org under a non-uuid id; a partner org carries a `Nice Academy - Math Academy …` variant; class titles, built from course titles, appear with differing case for the same grade; and, most often missed, **a course has many class sections with the identical title** (order ten for a high-school course, order forty for a grade-level course), several of them with no active enrollment. "The Algebra II class" is a set of classes on one course id, not one class. Joining or grouping on title merges or splits courses; picking the first class by title silently picks one section. Use `course.sourcedId`, and gather every class on it; the course id list is in `reference/timeback-math-academy-courses.json`. Measure: `GET .../courses/?filter=title~'Math Academy'&limit=100`, group by title, count titles with more than one id; `GET .../classes/?filter=title~'Math Academy'&limit=3000`, group by `course.sourcedId`, count classes per course, and group by lower-cased title, count titles that appear in more than one spelling.
...
3. **`pctCompleteApp` on an enrollment exists only after the first event that carries one arrives for THAT enrollment.** A freshly created enrollment has no percent; absence means no percent-bearing event since this enrollment began, not zero progress and not "never active" (the same student may have hundreds of results under earlier enrollments). The percent is as old as `pctCompleteAppUpdatedAt`, and it stops moving whenever the newest events lack the field: on a sample day about one Math Academy result in eleven carried no `pctCompleteApp`, so an active student's enrollment percent can sit a day or more behind their activity. Measure: enrollments in Math Academy classes with `status='active'` and no `metadata.pctCompleteApp`; and results in a window, count rows without `metadata.pctCompleteApp` over all rows.
...
9. **A null `score` is a task with zero correct answers, not an unreported one.** On a sample day every null-score Math Academy result carried `correctQuestions` 0 and `xp` at or below 0 (observed 2026-09-17; the earlier reading of "unreported" was wrong). Read `score: null` as 0 percent for accuracy purposes, and note the pattern: Timeback writes null rather than 0 when nothing was correct. Whether any other cause produces a null score is open (question #9). Measure: results with `metadata.appName='Math Academy'` in a window, count `score` null, and within those count `correctQuestions` not equal to 0.
...
11. **A course's `totalXp` can lag Math Academy's XP-with-reviews figure.** The school sets `course.metadata.metrics.totalXp` by hand from Math Academy's published per-course XP, so it can fall behind when Math Academy re-sizes a course; when it does, every XP-remaining estimate on that course reads low. On 2026-09-17 every course carrying a figure was checked against the school's table and matched after a correction the same day; three courses (the bare `Math Academy` course and both `Beyond AI` courses) carry no figure at all. Standing: **observed-empirically**. Measure: for one course, compare `course.metadata.metrics.totalXp` with the median over its active students of `enrollment.metadata.metrics.totalXp ÷ (pctCompleteApp/100)`; a course whose students' implied size sits well above its `totalXp` is one that has lagged. Never use `totalXp` as a denominator for progress; progress is `pctCompleteApp`.
12. **Test, shadow and demo accounts sit in real classes, and some courses hold only them.** `users.metadata.isTestUser` marks some; others are only recognisable by name patterns or by their org. Any count of Math Academy students includes them unless filtered. The bare `Math Academy` course and the `Beyond AI` courses are meant to hold no real student at all (owner rule, 2026-09-17), and a non-test enrollment there is a roster defect. Measure: students of a Math Academy class, count `metadata.isTestUser = true`; and for the three courses named, active in-window enrollments whose user is not a test user (expected: none).
...
19. **Filtering results by course is rejected.** `assessmentLineItem.course.sourcedId` in a results filter returns 400. Scope by course through line items (`assessmentLineItems?filter=course.sourcedId='…'`, or `filter=dateLastModified>='<day>'` for one day across every course, observed to be honoured 2026-09-17), or through the students of the course's classes.
20. **`/classes/{id}/students` over-reports the roster.** It returns every user who has ever held an enrollment in the class, including users whose only enrollment is `tobedeleted`; a section with zero active enrollments still returns users. Never take a roster from that route alone: take the roster from enrollments (`status='active'`, today in the date window) and use the students route only to resolve those ids to user records. Measure: for one class, count users from the students route against count of active in-window enrollments.
21. **Events can be filed under a deleted enrollment's course.** The line item's course is the enrollment the platform matched when the event arrived; when a student is moved, older events stay under the old course and some new ones can still land there. Joining events to the student's current enrollment mis-attributes them. Measure: for one day of Math Academy results, count rows whose line item course differs from the student's current active in-window enrollment course.
...
- **#9 — Whether anything other than zero correct answers produces `score: null`.** Observed 2026-09-17: every null-score row that day had zero correct (trap 9); the platform has not confirmed the rule.
